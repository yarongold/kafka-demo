...
- **Kafka CR rejected with `UnsupportedKafkaVersionException`.** The Strimzi community operator only supports a narrow window of Kafka versions per release. Check the supported list: `oc get kafka demo-kafka -n kafka-demo -o jsonpath='{.status.conditions[0].message}'`. Update `spec.kafka.version` in `k8s/kafka-cluster/kafka.yaml` to one of the listed versions and re-apply.
- **Producer/consumer pods stuck in `ImagePullBackOff` with `docker.io/library/<name>` in the error.** The deployment annotation `alpha.image.policy.openshift.io/resolve-names: "*"` is what tells OpenShift to rewrite the bare `producer:latest` / `consumer:latest` references to the internal registry. The included manifests set this; if you ever copy a deployment elsewhere, remember to carry the annotation along, or use the full registry URL.
- **New pods stuck `Pending` with `Insufficient memory` after a rollout.** CRC's single node can't always fit both the old and the new ReplicaSet's pods at once. Delete the old `ImagePullBackOff` / failing pods (`oc delete pod ...`) to free their requests so the new ones can schedule.
